...
# GITHUB_TOKEN=ghp_...           (rate limit GitHub × 10, requis pour Code Search)
...
| `github-skill-files` | `crawlGithubSkillFiles` | Code Search GitHub (token requis) |
| `github-agent-repo` | `crawlGithubAgentRepo` | Trees API, parse agents.md |
| `github-agent-files` | `crawlGithubAgentFiles` | Code Search GitHub (token requis) |
...
- `architecture`, `tech_stack`, `analyst_notes` retournés par `/goals/decompose` — affichés dans Goals.jsx ✓ (déjà fait dans cette version)
...
- Branche active : `claude/new-app-from-scratch-1byn46` sur `do4fun/copilot-custom-app-base`.
